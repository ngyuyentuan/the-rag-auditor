...
def unique_run_id():
    ...
    return f"robust_verify_fast_{stamp}_{suffix}"


def write_json_create(path, obj):
    path = Path(path)
    if path.exists():
        raise SystemExit(f"output path exists: {path}")
...
def read_parquet_sample(path, cols, fit_n, fit_seed):
    import pyarrow.parquet as pq
    import pandas as pd
    pf = pq.ParquetFile(path)
    total_rows = pf.metadata.num_rows
    if fit_n is None or fit_n >= total_rows:
        tables = [pf.read_row_group(i, columns=cols) for i in range(pf.num_row_groups)]
    ...
    target = int(fit_n * 1.2)
    rows = 0
    tables = []
    for idx in rg_indices:
        t = pf.read_row_group(idx, columns=cols)
        tables.append(t)
        rows += t.num_rows
        if rows >= target:
            break
...
def load_feature_frame_sampled(parquet_path, logit_col, feature_cols, fit_n, fit_seed):
    available = get_parquet_columns(parquet_path)
    cols = required_columns(feature_cols, logit_col, available)
    df = read_parquet_sample(parquet_path, cols, fit_n, fit_seed)
...
def build_feature_map_subset_filter(parquet_path, logit_col, feature_cols, qid_set):
    ...
    qids = list(qid_set)
    expr = ds.field("qid").isin(qids)
    table = dataset.to_table(columns=cols, filter=expr)
    df = table.to_pandas()
    if "qid" not in df.columns:
        raise SystemExit("missing qid in parquet")
    df["qid"] = df["qid"].astype(str)
    df = df.drop_duplicates("qid")
    if len(df) < len(qid_set):
        raise SystemExit(f"missing qids after filter; found={len(df)} needed={len(qid_set)}")
    feat_df = build_feature_frame(df, logit_col, feature_cols)
...
def fit_thresholds_fast(p_hat, y, alpha, beta, grid_points):
    p_hat = np.asarray(p_hat, dtype=float)
    y = np.asarray(y, dtype=int)
    qs = np.linspace(0.0, 1.0, grid_points)
    thr = np.unique(np.quantile(p_hat, qs))
    thr.sort()
    n = len(p_hat)
    ...
    total_pos = cum_pos[-1] if n else 0
    total_neg = cum_neg[-1] if n else 0
    best = None
    for t_accept in thr:
        idx_accept = np.searchsorted(p_hat[order], t_accept, side="left")
        accept_count = n - idx_accept
        fp = (total_neg - (cum_neg[idx_accept - 1] if idx_accept > 0 else 0)) / n if n else 0.0
        for t_reject in thr:
            if t_reject >= t_accept:
                break
            idx_reject = np.searchsorted(p_hat[order], t_reject, side="right") - 1
            if idx_reject < 0:
                reject_count = 0
                fn = 0.0
            else:
                reject_count = idx_reject + 1
                fn = (cum_pos[idx_reject]) / n if n else 0.0
            if fp > alpha or fn > beta:
                continue
            unc = 1.0 - (accept_count + reject_count) / n if n else 0.0
            ok = 1.0 - fp - fn
...
def main():
    ...
    ap.add_argument("--fit_n", type=int, default=2000)
    ap.add_argument("--grid_points", type=int, default=21)
    ...
    df_fit, feat_fit = load_feature_frame_sampled(parquet_path, base.logit_col, base.feature_cols, args.fit_n, 14)
    x_fit = feat_fit.to_numpy()
    y_fit = df_fit["y"].astype(int).to_numpy()
    p_hat = ensemble_probs([r.model for r in routers], flip_flags, x_fit)
    fitted = fit_thresholds_fast(p_hat, y_fit, 0.01, 0.01, args.grid_points)
    ...
    for row in baseline_rows:
        ...
        qid_set.add(qid)
    feature_map, features_present = build_feature_map_subset_filter(parquet_path, base.logit_col, base.feature_cols, qid_set)
    ...
    for budget in args.budgets:
        for seed in args.seeds:
            ...
            write_jsonl_create(sample_path, sample_rows(baseline_rows, args.n, seed))
            out_path = runs_dir / f"scifact_ens_seed{seed}_budget{budget:.3f}.jsonl"
            if out_path.exists():
                raise SystemExit(f"output path exists: {out_path}")
            if out_path.parent != runs_dir:
                raise SystemExit("output path must be under runs/sweeps/<run_id>")
            reuse_path = None
            if resume_root:
                reuse_path = resume_root / f"scifact_ens_seed{seed}_budget{budget:.3f}.jsonl"
                if reuse_path.exists():
                    stats = summarize(list(iter_jsonl(reuse_path)), "uncertain_only")
                    stats["seed"] = seed
                    stats["budget"] = budget
                    stats["reused"] = True
                    reused.append(str(reuse_path))
                    evaluated.append(stats)
                    metric_path = reports_dir / f"metrics_seed{seed}_budget{budget:.3f}.json"
                    write_json_create(metric_path, stats)
                    continue
    ...
    agg = aggregate_budget_rows(evaluated)
    fp_max = 0.01
    fn_max = 0.01
    ran_max = 0.25
    cap_max = 0.20
    ...
    partial = pair_count < (len(args.budgets) * len(args.seeds))
    lines = []
    lines.append("# Product Proof Router v2 SciFact Robust Verify (fast)")
    lines.append("")
    ...
    lines.append(f"- grid_points: {args.grid_points}")
    lines.append(f"- evaluated_pairs: {pair_count}")
    lines.append(f"- max_pairs: {args.max_pairs}")
    lines.append(f"- partial_results: {str(partial).lower()}")
    if reused:
        lines.append(f"- reused_paths: {', '.join(reused)}")
    lines.append("")
    lines.append("## Per-seed metrics")
    lines.append("")
    lines.append("| seed | budget | accept_rate | reject_rate | uncertain_rate | stage2_route_rate | stage2_ran_rate | capped_rate | fp_accept_rate | fn_reject_rate | ok_rate_stage1 | mean_ms | p95_ms | reused |")
    lines.append("|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|:---:|")
    for r in evaluated:
        lines.append("| {seed} | {b:.3f} | {a:.4f} | {rj:.4f} | {u:.4f} | {route:.4f} | {ran:.4f} | {cap:.4f} | {fp:.4f} | {fn:.4f} | {ok:.4f} | {mean:.2f} | {p95:.2f} | {reuse} |".format(
            seed=r["seed"],
            b=r["budget"],
            a=r["accept_rate"],
            rj=r["reject_rate"],
            ...
            mean=r["mean_ms"],
            p95=r["p95_ms"],
            reuse="yes" if r.get("reused") else "no",
        ))
    lines.append("")
