...
def average_probs(prob_list, flip_flags):
    acc = None
    for probs, flip in zip(prob_list, flip_flags):
        arr = np.asarray(probs, dtype=float)
        if flip:
            arr = 1.0 - arr
        acc = arr if acc is None else acc + arr
    return acc / float(len(prob_list))


def build_router_from_models(model_paths):
    ...
    for r in routers[1:]:
        ...
        if float(r.t_accept) != float(base.t_accept) or float(r.t_reject) != float(base.t_reject):
            raise SystemExit("ensemble thresholds mismatch")
    ...
    class EnsembleModel:
        ...
        def predict_proba(self, x):
            probs = [m.model.predict_proba(x)[:, 1] for m in self.models]
            avg = average_probs(probs, self.flips)
            return np.vstack([1.0 - avg, avg]).T

    class RouterWrapper:
        pass

    wrapper = RouterWrapper()
    wrapper.model = EnsembleModel(routers, flip_flags)
    wrapper.feature_cols = list(base.feature_cols)
    wrapper.logit_col = base.logit_col
    wrapper.t_accept = base.t_accept
    wrapper.t_reject = base.t_reject
...
def main():
    ...
    ap.add_argument("--budgets", type=float, nargs="+", default=[0.26, 0.27, 0.275, 0.28, 0.285, 0.29, 0.30])
    args = ap.parse_args()
    ...
    router = build_router_from_models(model_paths)
    qid_set = set()
    ...
    for seed in args.seeds:
        ...
        for budget in args.budgets:
            out_path = runs_dir / f"router_v2_budget_{budget:.3f}_seed_{seed}_{args.n}.jsonl"
            if out_path.exists():
    ...
    lines.append("")
    lines.append(f"- model: `{','.join(model_paths)}`")
