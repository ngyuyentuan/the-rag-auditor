...
def build_feature_map_subset_scan(parquet_path, logit_col, feature_cols, qid_set, max_scan_rows):
    ...
    found = {}
    scanned = 0
    for batch in dataset.to_batches(columns=cols, batch_size=8192):
        scanned += batch.num_rows
        df = batch.to_pandas()
        if "qid" not in df.columns:
            raise SystemExit("missing qid in parquet")
        df["qid"] = df["qid"].astype(str)
        sub = df[df["qid"].isin(qid_set)]
        if not sub.empty:
            for rec in sub.to_dict("records"):
                qid = str(rec.get("qid"))
                if qid and qid not in found:
                    found[qid] = rec
        if len(found) >= len(qid_set):
            break
        if scanned >= max_scan_rows:
            raise SystemExit(f"did not find all qids within scan limit; found={len(found)} needed={len(qid_set)}")
    if len(found) < len(qid_set):
        raise SystemExit(f"missing qids after scan; found={len(found)} needed={len(qid_set)}")
...
def fit_thresholds_fast(p_hat, y, alpha, beta, grid_points=51):
...
def main():
    ...
    ap.add_argument("--max_scan_rows", type=int, default=800000)
    args = ap.parse_args()
    ...
    df_fit, feat_fit = load_feature_frame_sampled(parquet_path, base.logit_col, base.feature_cols, args.fit_n, args.fit_seed)
    x_fit = feat_fit.to_numpy()
    y_fit = df_fit["y"].astype(int).to_numpy()
    p_hat = ensemble_probs([r.model for r in routers], flip_flags, x_fit)
    fitted = fit_thresholds_fast(p_hat, y_fit, 0.01, 0.01)
    ...
    for row in baseline_rows:
        ...
        qid_set.add(qid)
    feature_map, features_present = build_feature_map_subset_scan(parquet_path, base.logit_col, base.feature_cols, qid_set, args.max_scan_rows)
    ...
    lines.append(f"- max_scan_rows: {args.max_scan_rows}")
    lines.append("runtime mitigation: thresholds were fit on fit_n subsample; feature_map was built by bounded scan with early-stop once all qids were found; robust verify metrics are computed from n=200 runs per seed/budget")
    lines.append("")
