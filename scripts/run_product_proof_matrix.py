...
def has_tmp(path):
    return Path(str(path) + ".tmp").exists()
...
def main():
    ...
    ap.add_argument("--out_dir", default="runs/product_proof")
    ap.add_argument("--prod_yaml_scifact", default="configs/thresholds_stage1_prod_scifact.yaml")
    ap.add_argument("--prod_yaml_fever", default="configs/thresholds_stage1_prod_fever.yaml")
    args = ap.parse_args()
    ...
    def base_cmd(track, in_path, sample_path):
        cmd = [
            ...
            "16",
            "--out",
            str(out_dir / f"{track}_baseline_uncertain_only_{args.n}.jsonl"),
            "--in_path",
            in_path,
            "--sample",
            str(sample_path),
    ...
    scifact_dry = out_dir / f"scifact_baseline_uncertain_only_{args.n}_dry.jsonl"
    fever_dry = out_dir / f"fever_baseline_uncertain_only_{args.n}_dry.jsonl"
    ...
    if not scifact_dry.exists() and not has_tmp(scifact_dry):
        run(scifact_base + ["--dry_run_stage2", "--out", str(scifact_dry)])
    if not fever_dry.exists() and not has_tmp(fever_dry):
        run(fever_base + ["--dry_run_stage2", "--out", str(fever_dry)])

    if not scifact_u.exists() and not has_tmp(scifact_u):
        run(scifact_base + ["--out", str(scifact_u)])
    if not fever_u.exists() and not has_tmp(fever_u):
        run(fever_base + ["--out", str(fever_u)])

    if not scifact_a.exists() and not has_tmp(scifact_a):
        run(scifact_base + ["--stage2_policy", "always", "--out", str(scifact_a)])
    if not fever_a.exists() and not has_tmp(fever_a):
        run(fever_base + ["--stage2_policy", "always", "--out", str(fever_a)])

    scifact_prod_u = out_dir / f"scifact_prod_uncertain_only_{args.n}.jsonl"
    scifact_prod_a = out_dir / f"scifact_prod_always_{args.n}.jsonl"
    fever_prod_u = out_dir / f"fever_prod_uncertain_only_{args.n}.jsonl"
    fever_prod_a = out_dir / f"fever_prod_always_{args.n}.jsonl"

    if scifact_u.exists() and not scifact_prod_u.exists():
        run([
            ".venv/bin/python",
            "scripts/apply_stage1_prod_thresholds.py",
            "--in_jsonl",
            str(scifact_u),
            "--track",
            "scifact",
            "--config_yaml",
            args.prod_yaml_scifact,
            "--out_jsonl",
            str(scifact_prod_u),
        ])
    if scifact_a.exists() and not scifact_prod_a.exists():
        run([
            ".venv/bin/python",
            "scripts/apply_stage1_prod_thresholds.py",
            "--in_jsonl",
    ...
    if fever_u.exists() and not fever_prod_u.exists():
        run([
            ".venv/bin/python",
            "scripts/apply_stage1_prod_thresholds.py",
            "--in_jsonl",
            ...
            "fever",
            "--config_yaml",
            args.prod_yaml_fever,
            "--out_jsonl",
            str(fever_prod_u),
        ])
    if fever_a.exists() and not fever_prod_a.exists():
        run([
            ".venv/bin/python",
            "scripts/apply_stage1_prod_thresholds.py",
            "--in_jsonl",
    ...
    report = Path("reports/product_proof_matrix.md")
    report.parent.mkdir(parents=True, exist_ok=True)
    lines = []
    lines.append("# Product Proof Matrix")
    lines.append("")
    for track in ["scifact", "fever"]:
        ...
        for name in [
            ...
            f"{track}_prod_uncertain_only_{args.n}.jsonl",
            f"{track}_prod_always_{args.n}.jsonl",
            f"{track}_baseline_uncertain_only_{args.n}_dry.jsonl",
        ]:
            path = out_dir / name
            if not path.exists():
