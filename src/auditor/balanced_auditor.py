...
class BalancedAuditor:
    ...
    NEI_WORDS = [
        ...
        'hypothetically', 'theoretically', 'allegedly', 'reportedly',
        'debatable', 'questionable', 'speculative', 'tentative',
        'complex', 'complicated', 'nuanced', 'multifaceted',
        'context-dependent', 'case-by-case', 'on a case',
        'further research', 'more studies', 'remains to be seen',
        'not fully understood', 'not well established',
        'subject to interpretation', 'open question',
        'on one hand', 'on the other hand', 'however', 'although',
        'but', 'yet', 'while', 'whereas', 'despite',
    ]
...
ProductionAuditor = BalancedAuditor


if __name__ == "__main__":
    print("=" * 60)
    print("BALANCED AUDITOR TEST")
    print("=" * 60)
    ...
    for claim, evidence, expected in tests:
        ...
        print(f"{'Y' if match else 'N'} [{expected}->{result.verdict}] {claim[:40]}")
