...
def apply_constrained_thresholds(in_jsonl, out_jsonl, t_lower, t_upper, tau, policy):
...
def main():
    ...
    ap.add_argument("--out_dir", default="runs/product_proof_constrained")
    ap.add_argument("--constrained_yaml_scifact", default="configs/thresholds_stage1_constrained_scifact.yaml")
    ap.add_argument("--constrained_yaml_fever", default="configs/thresholds_stage1_constrained_fever.yaml")
    ap.add_argument("--out_md", default="reports/product_proof_constrained_matrix.md")
    args = ap.parse_args()
    ...
    import yaml
    sc_tau, sc_l, sc_u = load_yaml(args.constrained_yaml_scifact)
    fv_tau, fv_l, fv_u = load_yaml(args.constrained_yaml_fever)

    sc_prod_u = out_dir / f"scifact_constrained_uncertain_only_{args.n}.jsonl"
    sc_prod_a = out_dir / f"scifact_constrained_always_{args.n}.jsonl"
    fv_prod_u = out_dir / f"fever_constrained_uncertain_only_{args.n}.jsonl"
    fv_prod_a = out_dir / f"fever_constrained_always_{args.n}.jsonl"

    if scifact_u.exists() and not sc_prod_u.exists():
        apply_constrained_thresholds(scifact_u, sc_prod_u, sc_l, sc_u, sc_tau, "uncertain_only")
    if scifact_a.exists() and not sc_prod_a.exists():
        apply_constrained_thresholds(scifact_a, sc_prod_a, sc_l, sc_u, sc_tau, "always")
    if fever_u.exists() and not fv_prod_u.exists():
        apply_constrained_thresholds(fever_u, fv_prod_u, fv_l, fv_u, fv_tau, "uncertain_only")
    if fever_a.exists() and not fv_prod_a.exists():
        apply_constrained_thresholds(fever_a, fv_prod_a, fv_l, fv_u, fv_tau, "always")

    report = Path(args.out_md)
    report.parent.mkdir(parents=True, exist_ok=True)
    lines = []
    lines.append("# Product Proof Constrained Matrix")
    lines.append("")
    for track in ["scifact", "fever"]:
        ...
        for name in [
            ...
            f"{track}_constrained_uncertain_only_{args.n}.jsonl",
            f"{track}_constrained_always_{args.n}.jsonl",
        ]:
            path = out_dir / name
            if not path.exists():
