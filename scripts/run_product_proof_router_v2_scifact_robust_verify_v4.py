...
from src.stage1.router_v2 import load, build_feature_frame, decide_router
...
def load_feature_frame(parquet_path, logit_col, feature_cols):
    import pandas as pd
...
def fit_thresholds(p_hat, y, alpha_list, beta_list, objective):
            ...
            for t_accept in candidates:
                for t_reject in candidates:
                    ...
                    if objective == "min_route":
                        cand = (unc, -ok, t_accept, t_reject, alpha, beta)
                    elif objective == "min_uncertain":
                        cand = (unc, -ok, t_accept, t_reject, alpha, beta)
                    else:
                        cand = (unc, -ok, t_accept, t_reject, alpha, beta)
                    if best is None or cand < best[0]:
                        best = (cand, {"t_accept": float(t_accept), "t_reject": float(t_reject), "fp": fp, "fn": fn, "unc": unc, "ok": ok, "alpha": alpha, "beta": beta})
    if best is None:
        raise SystemExit("no feasible thresholds for given alpha/beta lists")
    return best[1]
...
def main():
    ...
    ap.add_argument("--seeds", type=int, nargs="+", default=[5, 12, 21, 33, 42, 57, 64, 77, 88, 99])
    ap.add_argument("--budgets", type=float, nargs="+", default=[0.26, 0.28, 0.30, 0.32, 0.34, 0.36])
    ap.add_argument("--alpha_list", type=float, nargs="+", default=[0.005, 0.008, 0.01])
    ap.add_argument("--beta_list", type=float, nargs="+", default=[0.005, 0.008, 0.01])
    ap.add_argument("--objective", default="min_route")
    ap.add_argument("--stage2_priority_mode", default="uncertainty")
    args = ap.parse_args()
    ...
    df, feat_df = load_feature_frame(parquet_path, base.logit_col, base.feature_cols)
    x = feat_df.to_numpy()
    y = df["y"].astype(int).to_numpy()
    p_hat = ensemble_probs([r.model for r in routers], flip_flags, x)
    fitted = fit_thresholds(p_hat, y, args.alpha_list, args.beta_list, args.objective)
    ...
    lines.append(f"- fit_alpha: {fitted['alpha']}")
    lines.append(f"- fit_beta: {fitted['beta']}")
    lines.append("")
    ...
    lines.append("")
    if best is None:
        lines.append("- no budget met fp_worst<=0.008 fn_worst<=0.008 stage2_ran_rate<=0.22 capped_rate<=0.15 on worst-case")
        penalties = []
        for budget, row in agg.items():
    ...
    report = reports_dir / "product_proof_router_v2_scifact_robust_verify.md"
