...
def main():
    ...
    lines = []
    lines.append(f"# Stage1 Tuning ({args.track})")
    lines.append("")
    ...
    if best_overall is None:
        ...
        out_yaml.write_text(yaml.safe_dump({
            "tau": float(tau),
            "t_lower": float(best_overall["t_lower"]),
            "t_upper": float(best_overall["t_upper"]),
            "seed": int(args.seed),
            "n": int(len(df)),
            "gates": {
                "max_fp_accept": fp_limits,
                "max_fn_reject": fn_limits,
                "max_uncertain": unc_limits,
            },
        }, sort_keys=False), encoding="utf-8")
